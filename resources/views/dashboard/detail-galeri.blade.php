...
<div class="card mb-3">
  <img class="img-fluid" src="{{ asset($galeri->gambar) }}" data-holder-rendered="true">
  <div class="card-body">
    ...
    <img alt="image" src="{{asset($galeri->user['gambar'])}}" class="rounded-circle mr-1" aria-expanded="false" v-pre style="width:50px">
    <small class="text-muted">Oleh : {{$galeri->user['name']}}</small></p>
  </div>
</div>

@if (Auth::check())
<form action="{{ url('/commentgaleri') }}" method="post">
@csrf
<input type="hidden" name="id" value="{{ $galeri->id }}" class="form-control">
...
<div class="card">
  ...
  <div class="col-md-12">
@foreach ($galeri->commentsgaleri as $row)
    <blockquote>
        <hr>
        <p>{{ $row->comment }}</p>
    ...
    @foreach ($row->childgaleri as $val) 
